  ...
  <el-dialog v-model="state.isShowDialog" destroy-on-close draggable title="证书" width="500">
    <el-form ref="dnsFormRef" label-width="auto" :model="state.form">
      ...
      <el-form-item label="Acme 账户" required prop="acmeId"
        :rules="[{ required: true, message: '域名 不能为空', trigger: 'change' }]">
      ...
      <!-- <el-form-item label="域名" required prop="primaryDomain"
        :rules="[{ required: true, message: '域名 不能为空', trigger: 'change' }]">
        <el-select v-model="state.form.acmeId" placeholder="请选择 域名" clearable>
          <el-option v-for="item in state.acmeList" :key="item.id" :label="item.description" :value="item.id ?? ''" />
        </el-select>
      </el-form-item> -->
      <el-form-item label="域名" required prop="domains"
        :rules="[{ required: true, message: '域名 不能为空', trigger: 'blur' }]">
        <el-input v-model="state.form.domains" placeholder="请填写 域名"  type="textarea" clearable />
      </el-form-item>
...
import client, { AcmeInfoDetailOutput, type ApiRequestDomainCreateDomainOrderRequestType } from '@/api';
...
const state = reactive({
  ...
  loading: false,
  isShowDialog: false,
  form: {} as ApiRequestDomainCreateDomainOrderRequestType
...
const submit = () => {
  dnsFormRef.value.validate(async (valid: boolean) => {
    if (!valid) return;
      await client.ApiRequestDomainCreateDomainOrder(state.form);
    closeDialog();
  });
};
