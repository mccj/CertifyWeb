<template>
  <el-dialog v-model="state.isShowDialog" destroy-on-close draggable title="Acme 账户" width="500">
    <el-form label-width="auto" :model="state.form" ref="accountFormRef">
      <el-form-item label="证书颁发机构" required prop="acmeId" :rules="[{ required: true, message: '证书颁发机构不能为空', trigger: 'change' }]">
        <el-select v-model="state.form.acmeId" placeholder="请选择 证书颁发机构" clearable>
          <el-option v-for="item in state.acmeList" :key="item.id" :label="`${item.title}【${item.standardExpiryDays}天】`" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="电子邮件" required prop="email" v-if="state.acmeList?.find(x=>x.id==state.form.acmeId)?.requiresEmailAddress" :rules="[{ required: true, message: '电子邮件不能为空', trigger: 'blur' }]">
      ...
      <template v-if="state.acmeList?.find(x=>x.id==state.form.acmeId)?.requiresExternalAccountBinding">
        ...
        <el-form-item label="Key(HMAC)">
          <el-input v-model="state.form.eabKey" required placeholder="请填写 eabKey" clearable />
        </el-form-item>
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    state.loading = true;
    var res = await client.ApiAcmeCertificateGetCertificateAuthorities(/*Object.assign({}, state.query, { pageNo: state.pagination.pageNo, pageSize: state.pagination.pageSize })*/);
    state.acmeList = res ?? [];
    state.loading = false;
  });
