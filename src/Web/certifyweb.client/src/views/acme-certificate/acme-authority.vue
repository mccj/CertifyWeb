<template>
  <div>
    <el-descriptions title="证书颁发机构" width="100%" style="width: 100%" :column="4">
      <template v-for="item in state.tableData">
        ...
        <el-descriptions-item label="描述" span="4">{{item.description}}</el-descriptions-item>
      </template>
    </el-descriptions>
    <!--<br>
...
<script setup lang="ts">
  ...
  const loadDatas = async () => {
    ...
    state.tableData = await client.GetCertificateAuthorities(/*Object.assign({}, state.query, { pageNo: state.pagination.pageNo, pageSize: state.pagination.pageSize })*/);
